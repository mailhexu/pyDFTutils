...
class lattice():
    def __init__(self,spacegroup,cellpars,species,positions,repeat=[1,1,1]):
    ...
    def inp_gen(self,outfile='spacegroup.in'):
        ...
        text=''
        text+="'%s'\n"%self.spacegroup
        text+=' '.join(map(str,[x/Bohr*Angstrom for x in self.cellpars[:3]]))+'\n'
    ...
    def xml_gen(self,outfile='spacegroup.xml'):
        ...
        lattice=etree.SubElement(root,'lattice',a=str(self.cellpars[0]),b=str(self.cellpars[1]),c=str(self.cellpars[2]),ab=str(self.cellpars[3]),ac=str(self.cellpars[4]),bc=str(self.cellpars[5]),ncell=' '.join([str(s) for s in self.repeat]))
        ...
        for s,p in zip(self.species,self.positions):
            ...
            ptree=etree.SubElement(stree, 'wpos',coord=' '.join((str(s) for s in p)))

        print((etree.tostring(root,pretty_print=True, xml_declaration=True,encoding="UTF-8")))
        if outfile:
            with open(outfile ,'w') as myfile:
                myfile.write(etree.tostring(root,pretty_print=True, xml_declaration=True,encoding="UTF-8"))
...
def test():
    a=lattice('Bmab',[10,10,24,90,90,90],['La','Cu','O','O'],[[0,0,0.36],[0,.0,0],[.25,.25,0],[0,0,0.18]])
    print(a.inp_gen())
    atoms=a.gen_atoms()
    return atoms
