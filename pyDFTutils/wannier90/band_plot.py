...
def plot_band_weight_file(fname='wannier90.up_band.dat', efermi=None,weight=True,yrange=None,output=None,style='alpha',color='blue',axis=None,width=10):
    """
    ...
    return plot_band_weight(kslist,ekslist,wkslist=wkslist,efermi=efermi,yrange=yrange,output=output,style=style,color=color,axis=axis,width=width,xticks=xticks)
...
def main():
    ...
    parser.add_argument('-y','--yrange',type=float ,nargs='+',help='range of yticks',default=None)
    parser.add_argument('-s','--style',type=str,help='style of line, width | alpha',default='width')
    args=parser.parse_args()
    if args.output is None:
        output=os.path.splitext(args.fname)[0]+'.png'
    if args.efermi is None:
        efermi=get_fermi('PDOS/OUTCAR')
    plot_band_weight_file(fname=args.fname,efermi=efermi,weight=args.weight,yrange=args.yrange,style=args.style)
