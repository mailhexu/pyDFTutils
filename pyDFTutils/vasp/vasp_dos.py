...
orb_t2g_label = ['$d_{xy}$+', '$d_{yz}$+',
                 '$d_{xz}$+', '$d_{xy}$-', '$d_{yz}$-', '$d_{xz}$-']
...
def get_total_dos(filename='DOSCAR', nspin=2, efermi=None):
    ...
    for line in dos_text:
        ...
        if line_num >= nbands+6:
            break
        if is_spin:
            e, dos_up, dos_down, idos_up, idos_down = [
                float(fstr) for fstr in line.split()]
            e_norm = e-efermi
...
def plot_dos(filename='DOSCAR', epmin=-20, epmax=5.0, output=None):
    ...
    """
    if is_spin_dos():
        plot_dos_spin(filename=filename, epmin=epmin,
                      epmax=epmax, output=output)
    else:
        plot_dos_nospin(filename=filename, epmin=epmin,
                        epmax=epmax, output=output)
...
def write_all_sum_dos(output='sum_dos.txt', nospin_output='sum_dos_nospin.txt', eg_polar_output='eg_polor.txt', erange=None):
    ...
    with open(output, 'w') as myfile:
        ...
        for symnum in symdict:
            sdoss = list(mydos.sum_dos_all(
                symdict[symnum], erange=erange).values())
            myfile.write('%s\t%s\n' % (symnum.ljust(
                6), '\t'.join(["%.3f" % s for s in sdoss])))

    orbs = mydos.get_orbital_names_nospin()
    with open(nospin_output, 'w') as myfile:
        myfile.write('symnum '+'\t'.join([x.ljust(5) for x in orbs])+'\n')
        for symnum in symdict:
            sdoss = list(mydos.sum_dos_all_nospin(
                symdict[symnum], erange=erange).values())
            myfile.write('%s\t%s\n' % (symnum.ljust(
                6), '\t'.join(["%.3f" % s for s in sdoss])))
    ...
    try:
        if eg_polar_output is not None:
            with open(eg_polar_output, 'w') as myfile:
                myfile.write(
                    'symnum '+'\t'.join(['dx2+', 'dx2-', 'dz2+', 'dz2-', 'dx2', 'dz2', 'rate'])+'\n')
                for symnum in symdict:
                    dx2_up = mydos.sum_dos(
                        symdict[symnum], 'dx2+', erange=erange)
                    dx2_down = mydos.sum_dos(
                        symdict[symnum], 'dx2-', erange=erange)
                    dz2_up = mydos.sum_dos(
                        symdict[symnum], 'dz2+', erange=erange)
                    dz2_down = mydos.sum_dos(
                        symdict[symnum], 'dz2-', erange=erange)

                    dx2 = mydos.sum_dos_nospin(
                        symdict[symnum], 'dx2', erange=erange)
                    dz2 = mydos.sum_dos_nospin(
                        symdict[symnum], 'dz2', erange=erange)
                    ...
                    myfile.write('%s\t%s\n' % (symnum.ljust(6), '\t'.join(["%.3f" % s for s in [
                                 dx2_up, dx2_down, dz2_up, dz2_down, dx2, dz2, porlar_rate]])))
...
class MyVaspDos(VaspDos):
    ...
    def get_orbital_names(self):
        ...
        orb_names = [x for x in norb if not(
            x.endswith('up') or x.endswith('down'))]
        return orb_names
...
def getldos(iatom, sites, location='./', doscar='DOSCAR', efermi='auto', is_up_major=False, return_efermi=False):
    ...
    if isinstance(iatom, str):
        try:
            ...
            print(exc)
            raise ValueError(
                'iatom should be int. Or a proper symnum with a POSCAR should be supplied')
...
def read_sumdos(filename='sum_dos.csv'):
    ...
    ncol = len(orbs)+1
    a = np.loadtxt(fname=filename, dtype=float, skiprows=1,
                   usecols=list(range(1, ncol)))
...
def plot_all_ldos(filename='DOSCAR', ispin=2, ymin=-2.0, ymax=2.0, xmin=-15.0, xmax=5.0, element_types=None, has_f=False):
    ...
    if element_types is not None:
        atom_nums = [x for x in list(
            symdict.keys()) if symnum_to_sym(x) in element_types]
    else:
    ...
    for atom_num in atom_nums:
        ...
        plotldos_group([atom_num], sites, ymin=ymin, ymax=ymax, xmin=xmin,
                       xmax=xmax, special_location=None, output='LDOS/%s_ldos.png' % atom_num)
...
def plot_pdos(sites, lsites=None, element_types=None, filename='DOSCAR', ispin=2, ymin=-2.0, ymax=2.0, xmin=-15.0, xmax=5.0, output_dir='PDOS'):
    ...
    for atom_num in atom_nums:
        ...
        plotldos_group([atom_num], lsites, ymin=ymin, ymax=ymax, xmin=xmin, xmax=xmax,
                       special_location=None, output=join(output_dir, '%s_%s_dos.png' % (atom_num, sites)))
...
def plot_all_pdos_eg(element_types=None, filename='DOSCAR', ispin=2, ymin=-2.0, ymax=2.0, xmin=-15.0, xmax=5.0):
    ...
    for atom_num in atom_nums:
        ...
        plotldos_group([atom_num], sites, ymin=ymin, ymax=ymax, xmin=xmin, xmax=xmax,
                       special_location=None, output='PDOS/%s_eg_dos.png' % atom_num)
...
def plot_all_pdos_t2g(element_types=None, filename='DOSCAR', ispin=2, ymin=-2.0, ymax=2.0, xmin=-15.0, xmax=5.0):
    ...
    for atom_num in atom_nums:
        copyfile(filename, 'PDOS/DOSCAR')
        plotldos_group([atom_num], sites, ymin=ymin, ymax=ymax, xmin=xmin, xmax=xmax,
                       special_location=None, output='PDOS/%s_t2g_dos.png' % atom_num)
...
def plot_all_pdos_p(element_types=None, filename='DOSCAR', ispin=2, ymin=-2.0, ymax=2.0, xmin=-15.0, xmax=5.0):
    ...
    for atom_num in atom_nums:
        copyfile(filename, 'PDOS/DOSCAR')
        plotldos_group([atom_num], sites, ymin=ymin, ymax=ymax, xmin=xmin,
                       xmax=xmax, special_location=None, output='PDOS/%s_p_dos.png' % atom_num)
...
def plot_all_pdos_s(element_types=None, filename='DOSCAR', ispin=2, ymin=-2.0, ymax=2.0, xmin=-15.0, xmax=5.0):
    ...
    for atom_num in atom_nums:
        copyfile(filename, 'PDOS/DOSCAR')
        plotldos_group([atom_num], sites, ymin=ymin, ymax=ymax, xmin=xmin,
                       xmax=xmax, special_location=None, output='PDOS/%s_s_dos.png' % atom_num)
...
def plot_all_pdos(element_types=None, filename='DOSCAR', ispin=2, ymin=-2.0, ymax=2.0, xmin=-15.0, xmax=5.0, output_dir='PDOS', orbs=['s', 'p', 'eg', 't2g']):
    for site in orbs:
        plot_pdos(sites=site, element_types=element_types, filename=filename, ispin=ispin,
                  ymin=ymin, ymax=ymax, xmin=xmin, xmax=xmax, output_dir=output_dir)
...
class single_dos_plot():
    ...
    def get_doses(self):
        for info in self.get_dos_info():
            ...
            energy, doses = getldos(sym_num, [
                                    orbital], location=path, doscar=filename, efermi='auto', is_up_major=False)
...
def plotldos_group(atom_num, sites, ymin=0.0, ymax=2.0, xmin=-15.0, xmax=5.0, special_location=None, output=None):
    ...
    for i, ss in enumerate(atom_num):
        if (special_location is None) or (i not in special_location):
            ...
            symdict_sp = get_symdict(filename=join(
                special_location[i][0], 'POSCAR'))
            energy, dos = getldos(
...
if __name__ == '__main__':
    ...
    names = ['Bi1', 'Fe1', 'O1', 'O3']
    # plotldos_group(names,['s+','p+','d+','s-','p-','d-'],ymin=[-0.7,-6,-0.7,-0.7],ymax=[0.7,6,0.7,0.7])
    plotldos_group(names, ['s+', 'p+', 'd+', 'f+', 's-', 'p-', 'd-',
                   'f-'], ymin=[-0.7, -6, -0.7, -0.7], ymax=[0.7, 6, 0.7, 0.7])
